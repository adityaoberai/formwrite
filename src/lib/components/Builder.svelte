<script lang="ts">
	...
	let {
		...
		logoUrl?: string;
		disabled?: boolean;
		/** Rendered at the top of the canvas on small screens, where the page header is hidden. */
		header?: Snippet;
	} = $props();
	...
	// Below the lg breakpoint the palette and inspector open as dialogs instead of side panes.
	const DESKTOP = '(min-width: 1024px)';
	const isDesktop = () => typeof window !== 'undefined' && window.matchMedia(DESKTOP).matches;
	$effect(() => {
		const mq = window.matchMedia(DESKTOP);
		const sync = () => {
			if (mq.matches) inspectorOpen = false;
		};
	...
	function select(id: string) {
		selectedId = id;
		if (!isDesktop()) inspectorOpen = true;
	}
	...
	function addOption() {
		...
		const at = selected.options.length - 1;
		queueMicrotask(() => document.getElementById(`insp-opt-${at}`)?.focus());
	}
...
{#snippet palette()}
	{#each FIELD_GROUPS as group (group)}
		<p class="px-1.5 pt-2 pb-1.5 text-[11px] font-semibold tracking-wide text-stone-400 uppercase">
			{group}
		</p>
		<div class="mb-1 grid gap-0.5 sm:grid-cols-2 lg:grid-cols-1">
		...
		<div class="space-y-4">
			...
			{#if FIELD_TYPES_WITH_OPTIONS.includes(selected.type)}
				<div>
					...
					<button type="button" class="btn btn-secondary btn-sm mt-2" onclick={addOption} {disabled}
						><Icon name="plus" size={13} /> Add option</button
					>
				</div>
			{/if}
...
<div class="flex h-full min-h-0 flex-col">
	...
	<div class="flex items-center justify-between gap-3 border-b border-stone-200 bg-white px-4 py-2">
		...
		<div class="hidden text-xs text-stone-500 sm:block">
	...
	{:else}
		<div class="grid min-h-0 flex-1 lg:grid-cols-[236px_minmax(0,1fr)_320px]">
			<!-- Palette (desktop sidebar; on small screens it opens as a dialog below) -->
			<aside
				class="hidden overflow-auto border-r border-stone-200 bg-white p-3 lg:block"
			...
			<section class="min-h-0 overflow-auto bg-stone-100/60 p-4 md:p-6" aria-label="Form canvas">
				<div class="mx-auto max-w-2xl space-y-2">
					{#if header}
						<div class="mb-4 rounded-xl border border-stone-200 bg-white p-4 shadow-card lg:hidden">
							{@render header()}
						</div>
					{/if}
					{#if fields.length === 0}
						<div class="rounded-2xl border-2 border-dashed border-stone-300 p-10 text-center">
							...
							<p class="mt-1 text-sm text-stone-500">
								<span class="hidden lg:inline">Pick a field type from the left to get started.</span
								>
								<span class="lg:hidden">Add your first field to get started.</span>
							</p>
							{#if !disabled}
								<button
									type="button"
										...
										{:else if FIELD_TYPES_WITH_OPTIONS.includes(field.type)}
											<div class="mt-2 flex flex-wrap gap-1.5">
												{#each (field.options ?? []).slice(0, 4) as opt, oi (oi)}
													<span
														class="rounded-md border border-stone-200 bg-stone-50 px-2 py-0.5 text-xs {opt
															? 'text-stone-600'
															: 'text-stone-400 italic'}">{opt || `Option ${oi + 1}`}</span
													>
												{/each}
			...
			<!-- Inspector -->
			<aside
				class="hidden min-h-0 overflow-auto border-l border-stone-200 bg-white p-4 lg:block"
		...
		<!-- Small screens: the palette opens as a dialog instead of a sidebar -->
		<Modal
			bind:open={paletteOpen}
			title="Add a field"
			description="It goes right after the selected card."
			size="md"
		>
			<div class="-mx-2 max-h-[60vh] overflow-auto px-2">
				{@render palette()}
			</div>
		</Modal>

		<!-- Small screens: tapping a card opens its settings as a dialog -->
		{#if selected}
			{@const meta = fieldMeta(selected.type)}
			<Modal bind:open={inspectorOpen} title={meta.label} description={meta.description} size="md">
				<div class="-mx-2 max-h-[65vh] overflow-auto px-2">
					{@render inspector()}
				</div>
				{#snippet footer()}
					<button type="button" class="btn btn-primary" onclick={() => (inspectorOpen = false)}
						>Done</button
					>
				{/snippet}
			</Modal>
